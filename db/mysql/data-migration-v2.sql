...
SELECT uuid() event_id,
    ...
    s.session_uuid,
    p.created_at,
    SUBSTRING_INDEX(url, '?', 1) AS url_path,
    IF(LOCATE('?', url) > 0, SUBSTRING_INDEX(url, '?', -1), '') AS url_query,
    SUBSTRING(SUBSTRING_INDEX(SUBSTRING_INDEX(referrer, '?', 1), '://', -1), LOCATE('/', SUBSTRING_INDEX(SUBSTRING_INDEX(referrer, '?', 1), '://', -1))) referrer_path,
    IF(LOCATE('?', referrer) > 0, SUBSTRING_INDEX(referrer, '?', -1), '') AS referrer_query,
    SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(referrer, '/', 3), '://', -1), '/', 1), '?', 1), ':', 1), 'www.', -1) referrer_domain,
    1 event_type
FROM v1_pageview p
JOIN v1_session s
